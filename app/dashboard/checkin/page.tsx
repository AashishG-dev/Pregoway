...
import { ChevronRight, ChevronLeft, Mic, Camera, Check } from "lucide-react";
...
export default function CheckInPage() {
  ...
  // Flattened flow logic would be more complex, simplification for prototype:
  // We just iterate the main array. If Yes/No triggers follow up, we show it (modal or next step).
  // For MVP, sticking to linear flow.
  ...
  const handleNext = async () => {
    if (step < questions.length) {
      ...
      if (authUser) {
        try {
          // Calculate streak (mock logic for now: query last checkin and add 1)
          const { error } = await supabase.from('daily_checkins').insert({
            user_id: authUser.id,
            data: answers,
            date: new Date().toISOString().split('T')[0],
            streak_count: 1 // TODO: calculated streak
          });
